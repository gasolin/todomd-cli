  ...
  async run(command: string, args: string[]): Promise<string | Task[]> {
    ...
    switch (effectiveCommand) {
      ...
      case ValidCommands.ListConAlias:
        ...
        if (!contextFilter) return 'Error: Please provide a context'
        const filteredByContext = tasks.filter((t) =>
          t.contexts?.includes(contextFilter)
        )
      ...
      case ValidCommands.ListPriAlias:
        ...
        if (filteredByPriority.length === 0)
          return `No tasks found for priority "(${priorityFilter})"`
        return filteredByPriority

      case ValidCommands.ListProj:
      case ValidCommands.ListProjAlias:
        const projectFilter = effectiveArgs[0]
        if (!projectFilter) return 'Error: Please provide a project'
        const filteredByProject = tasks.filter((t) =>
          t.projects?.includes(projectFilter)
        )
