...
export async function setupTestDirectory(): Promise<string> {
  ...
  await execPromise(`node ${cliPath} init`, { env: { ...process.env, TODO_DIR: tempDir } });
  return tempDir;
}
...
export async function addTask(dir: string, task: string): Promise<any> {
  return execPromise(`node ${cliPath} add "${task}"`, { env: { ...process.env, TODO_DIR: dir } });
}

export async function listTasks(dir: string): Promise<any> {
  return execPromise(`node ${cliPath} list`, { env: { ...process.env, TODO_DIR: dir } });
}
