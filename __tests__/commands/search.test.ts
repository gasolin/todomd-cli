...
describe('search command', () => {
  ...
  test('should find tasks matching the search term', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} search "special"`, { env: { ...process.env, TODO_DIR: tempDir } });
  ...
  test('should show a message if no tasks match', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} search "nonexistent"`, { env: { ...process.env, TODO_DIR: tempDir } });
