...
describe('listcon command', () => {
  ...
  test('should list only tasks with the specified context', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} listcon work`, { env: { ...process.env, TODO_DIR: tempDir } });
  ...
  test('should show a message if no tasks match the context', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} listcon nonexistent`, { env: { ...process.env, TODO_DIR: tempDir } });
  ...
  test('should list all unique contexts when no context is specified', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} listcon`, { env: { ...process.env, TODO_DIR: tempDir } });
