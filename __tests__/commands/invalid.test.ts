...
describe('Invalid Command', () => {
  ...
  test('should show an error for an unknown command', async () => {
    const { stdout } = await execPromise(`node ${cliPath} foobar`, { env: { ...process.env, TODO_DIR: tempDir } });
    expect(stdout).toContain('Error: Unknown command "foobar"');
    expect(stdout).toContain("Run 'todomd --help' to see a list of available commands.");
  });
  ...
  test('should show an error for an unknown command after a valid path', async () => {
    ...
    const { stdout } = await execPromise(`node ${cliPath} todo.md baz`, { cwd: tempDir });
