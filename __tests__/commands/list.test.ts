...
describe('list command', () => {
  ...
  test('should align task numbers correctly when there are more than 9 tasks', async () => {
    ...
    expect(lines.find(line => line.includes('Task 1'))).toMatch(/^ 1\. - \[ \] Task 1 cr:\d{4}-\d{2}-\d{2}/);
    // Task 10 should not be padded
    expect(lines.find(line => line.includes('Task 10'))).toMatch(/^10\. - \[ \] Task 10 cr:\d{4}-\d{2}-\d{2}/);
    // Task 11 should not be padded
    expect(lines.find(line => line.includes('Task 11'))).toMatch(/^11\. - \[ \] Task 11 cr:\d{4}-\d{2}-\d{2}/);
  });
});
