...
describe('Command Aliases', () => {
  ...
  test('alias "lsa" should work for "listall"', async () => {
    await execPromise(`node ${cliPath} add "Incomplete task for lsa"`, { env: { ...process.env, TODO_DIR: tempDir } });
    await execPromise(`node ${cliPath} add "Completed task for lsa"`, { env: { ...process.env, TODO_DIR: tempDir } });
    await execPromise(`node ${cliPath} done 2`, { env: { ...process.env, TODO_DIR: tempDir } });

    const { stdout } = await execPromise(`node ${cliPath} lsa`, { env: { ...process.env, TODO_DIR: tempDir } });
  ...
  test('alias "lsc" should work for "listcon"', async () => {
    await execPromise(`node ${cliPath} add "A task for lsc @work"`, { env: { ...process.env, TODO_DIR: tempDir } });
  ...
  test('alias "lsproj" should work for "listproj"', async () => {
    await execPromise(`node ${cliPath} add "A task for lsproj +work"`, { env: { ...process.env, TODO_DIR: tempDir } });
    const { stdout } = await execPromise(`node ${cliPath} lsproj work`, { env: { ...process.env, TODO_DIR: tempDir } });
